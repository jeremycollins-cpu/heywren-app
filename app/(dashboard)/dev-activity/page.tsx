...
import { GitCommit, GitPullRequest, Eye, Cpu, Clock, TrendingUp, ExternalLink, Info, ArrowUpRight, Copy, AlarmClock, BarChart3, Sparkles } from 'lucide-react'
...
interface RefactorRatioSummary {
  refactor_prs: number
  total_prs: number
  ratio: number | null
  top_refactors: Array<{
...
// ── Code volume sparkline (capacity sensing, not a leaderboard) ──
...
function CodeVolumeCard({ volume }: { volume: VolumeSummary }) {
  // Hide entirely if there's nothing to show — avoids a card full of zeros.
  ...
  // Trend language relative to the prior-weeks median (robust to one-off spikes).
  let trend = ''
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
      ...
      {/* Sparkline: stacked additions (green) + deletions (red) per week */}
          ...
          return (
            <div key={w.week_start} className="group relative flex-1 min-w-0 flex flex-col justify-end h-full">
              {w.total_lines > 0 ? (
                <>
                  {delPct > 0 && (
                    <div className="w-full bg-rose-400 dark:bg-rose-500/80" style={{ height: `${delPct}%` }} />
                  )}
                  {addPct > 0 && (
                    <div className="w-full bg-emerald-400 dark:bg-emerald-500/80 rounded-t-sm" style={{ height: `${addPct}%` }} />
                  )}
                </>
              ) : (
                <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-t-sm" style={{ height: '2%' }} />
...
function RefactorRatioCard({ refactor }: { refactor: RefactorRatioSummary }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
      ...
      <div className="flex items-baseline gap-2">
        ...
        <span className="text-xs text-gray-500 dark:text-gray-400">
          of merged PRs removed more than they added
        </span>
      </div>
      ...
      <div className="px-4 sm:px-6 py-6 max-w-[1200px] mx-auto space-y-6">
        ...
        {!data || (data.summary.totalCommits === 0 && data.summary.totalPrsOpened === 0) ? (
          ...
          <>
            ...
            {/* Code volume + refactor ratio (two-up) */}
            {data.prMetrics && (
