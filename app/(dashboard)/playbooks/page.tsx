...
import { FileText, Plus, CheckCircle2, Trash2, X, Zap, ToggleLeft, ToggleRight, Sparkles } from 'lucide-react'
...
export default function PlaybooksPage() {
  ...
  const [quickRuleSubmitting, setQuickRuleSubmitting] = useState(false)

  const QUICK_RULE_SUGGESTIONS = [
    "When a commitment is overdue for 3+ days, send a nudge",
    "Send a Slack message when new commitments are created",
    "Escalate to manager if a nudge is ignored",
    "Create a draft follow-up for stale items",
    "Send a daily email digest of open commitments",
  ]

  function parseNaturalRule(input: string): {
    name: string
    description: string
    trigger: string
    action: string
    ...
    let trigger = 'commitment_overdue'
    if (lower.includes('overdue')) {
      trigger = 'commitment_overdue'
    } else if (lower.includes('created') || lower.includes('new commitment')) {
      trigger = 'commitment_created'
    } else if (lower.includes('stale') || lower.includes('inactive')) {
      trigger = 'commitment_stale'
    } else if (lower.includes('meeting') || lower.includes('before meeting')) {
      trigger = 'meeting_upcoming'
    } else if (lower.includes('ignored') || lower.includes('no response')) {
      trigger = 'nudge_ignored'
    } else if (lower.includes('daily') || lower.includes('every day')) {
      trigger = 'daily_schedule'
    }

    let action = 'send_nudge'
    if (lower.includes('nudge') || lower.includes('remind')) {
      action = 'send_nudge'
    } else if (lower.includes('slack') || lower.includes('message')) {
      action = 'send_slack'
    } else if (lower.includes('email') || lower.includes('send email')) {
      action = 'send_email'
    } else if (lower.includes('draft') || lower.includes('follow-up') || lower.includes('followup')) {
      action = 'create_draft'
    } else if (lower.includes('escalate') || lower.includes('manager')) {
      action = 'escalate'
    } else if (lower.includes('reassign') || lower.includes('delegate')) {
      action = 'reassign'
    }

    const truncated = input.trim().length > 60 ? input.trim().slice(0, 57) + '...' : input.trim()
    const name = truncated.charAt(0).toUpperCase() + truncated.slice(1)

    return { name, description: input.trim(), trigger, action }
  }

  async function handleQuickRule() {
    if (!quickRuleInput.trim()) {
      toast.error('Please type a rule first')
      return
    }
    if (!teamId || !userId) {
    ...
    const parsed = parseNaturalRule(quickRuleInput)

    try {
      setQuickRuleSubmitting(true)
      ...
      setQuickRuleInput('')
      toast.success(`Rule created: ${parsed.trigger} → ${parsed.action}`)
    } catch (err) {
      console.error('Error creating quick rule:', err)
      toast.error('Failed to create rule')
    } finally {
      setQuickRuleSubmitting(false)
    }
  }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {/* Quick Rule */}
      <div className="bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-900/20 dark:to-violet-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          <Sparkles aria-hidden="true" className="w-5 h-5 text-indigo-500" />
          <h2 className="text-base font-semibold text-indigo-900 dark:text-indigo-200">Quick Rule</h2>
        </div>
        <div className="flex gap-3">
          <input
            type="text"
            value={quickRuleInput}
            onChange={(e) => setQuickRuleInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && !quickRuleSubmitting && handleQuickRule()}
            placeholder={`Type a rule in plain English, e.g. "When a commitment is overdue for 3 days, send a Slack reminder"`}
            className="flex-1 px-4 py-2.5 border border-indigo-200 dark:border-indigo-700 rounded-lg bg-white dark:bg-surface-dark text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm"
            disabled={quickRuleSubmitting}
            aria-label="Plain-English rule input"
          />
          <button
            onClick={handleQuickRule}
            disabled={quickRuleSubmitting || !quickRuleInput.trim()}
            className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
          >
            {quickRuleSubmitting ? 'Creating...' : 'Create Rule'}
          </button>
        </div>
        <div className="flex flex-wrap gap-2 mt-3">
          <span className="text-xs text-indigo-600 dark:text-indigo-400 font-medium self-center mr-1">Try:</span>
          {QUICK_RULE_SUGGESTIONS.map((suggestion) => (
            <button
              key={suggestion}
              onClick={() => setQuickRuleInput(suggestion)}
              className="text-xs px-3 py-1.5 rounded-full bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark text-gray-600 dark:text-gray-300 hover:border-indigo-300 dark:hover:border-indigo-600 hover:text-indigo-700 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition"
            >
              {suggestion}
            </button>
          ))}
