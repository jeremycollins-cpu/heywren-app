  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.success('Sync complete! Found ' + data.summary.commitments_detected + ' commitments.')
      }
    } catch (err) {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Pull in your Slack message history and scan for commitments, tasks, and follow-ups
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="mt-6">
          <button
            ...
            style={{
              background: syncing ? '#9CA3AF' : 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)',
              boxShadow: syncing ? 'none' : '0 2px 8px rgba(79, 70, 229, 0.25)',
            }}
          >
            {syncing ? (
              <>
        ...
        {result && (
          <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-green-900 text-sm">Sync Complete</h3>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-2xl font-bold text-green-900">{result.channels_processed}</p>
                <p className="text-xs text-green-700">Channels scanned</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-900">{result.messages_scanned}</p>
                <p className="text-xs text-green-700">Messages analyzed</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-900">{result.commitments_detected}</p>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-5">
        ...
        <p className="text-xs text-indigo-700 leading-relaxed">
          HeyWren reads messages from Slack channels where the bot has been invited.
          Each message is analyzed by AI to detect commitments, promises, and tasks.
          Results appear on your Dashboard and Commitments page. The bot only has read
          access and never posts or modifies anything in Slack.
        </p>
      </div>
    </div>
  )
